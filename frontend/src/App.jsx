  ...
  return (
    <div className="min-h-screen bg-slate-950 flex items-center justify-center px-4">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute top-2/3 left-1/3 w-[400px] h-[400px] bg-purple-600/10 rounded-full blur-3xl" />
      </div>
      ...
      <div className="relative w-full max-w-md">
        ...
        <div className="bg-slate-900/80 border border-slate-800 rounded-2xl p-8 shadow-2xl backdrop-blur">
          <form onSubmit={handleLogin} className="space-y-5">
            ...
            {/* Error */}
            {error && (
              <div className="flex items-center space-x-2 p-3 bg-rose-950/30 border border-rose-500/30 rounded-xl text-rose-400 text-sm">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full py-2.5 bg-indigo-600 hover:bg-indigo-500 disabled:bg-slate-700 text-white font-semibold text-sm rounded-xl transition flex items-center justify-center space-x-2 shadow-lg shadow-indigo-600/20"
            >
              {loading ? (
                <>
  ...
  const getRecBadge = (rec) => {
    switch (rec) {
      case "HIGHLY_RECOMMENDED":
        return <span className="px-2.5 py-0.5 rounded-full bg-emerald-500/10 text-emerald-400 border border-emerald-500/30 text-xs font-semibold">🔥 Highly Recommended</span>;
      case "RECOMMENDED":
        return <span className="px-2.5 py-0.5 rounded-full bg-indigo-500/10 text-indigo-400 border border-indigo-500/30 text-xs font-semibold">👍 Recommended</span>;
      case "NOT_RECOMMENDED":
        return <span className="px-2.5 py-0.5 rounded-full bg-rose-500/10 text-rose-400 border border-rose-500/30 text-xs font-semibold">⚠️ Low Match</span>;
      default:
        return <span className="px-2.5 py-0.5 rounded-full bg-amber-500/10 text-amber-400 border border-amber-500/30 text-xs font-semibold">🤔 Maybe</span>;
    }
  };
        ...
        <div className="max-w-7xl mx-auto px-4 py-4 flex justify-between items-center">
          ...
          <div className="flex items-center space-x-3">
            ...
            >
              ...
              <span>{scraping ? "Scraping..." : "Trigger Scraper"}</span>
            </button>
            <button
              onClick={onLogout}
              className="flex items-center space-x-1.5 px-3 py-2 text-slate-400 hover:text-slate-200 hover:bg-slate-800 border border-slate-700 rounded-lg text-sm transition"
            >
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between text-xs">
                      <div className="flex items-center space-x-2">
                        ...
                        {isApplied ? (
                          <span className="px-2 py-0.5 rounded bg-emerald-500/20 text-emerald-400 font-medium border border-emerald-500/30">Applied</span>
                        ) : (
                          <span className="px-2 py-0.5 rounded bg-slate-800 text-slate-400">Not Applied</span>
                        )}
                      </div>
                      <button
            ...
            <div className="bg-slate-900/60 border border-slate-800 rounded-xl p-6 space-y-5">
              ...
              <div className="p-4 rounded-xl bg-slate-950/80 border border-slate-800">
                <div className="flex justify-between items-center mb-2">
                  <span className="text-xs font-semibold uppercase tracking-wider text-slate-400">Match Compatibility Score</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800">
                  <span className="text-xs font-semibold uppercase text-slate-400 tracking-wider flex items-center mb-2">
                    <Briefcase className="w-3.5 h-3.5 mr-1 text-amber-400" /> Required Experience
                  </span>
                  <p className="text-sm text-slate-200">{selectedJob.required_experience || "Not explicitly specified"}</p>
                </div>
                <div className="p-4 rounded-xl bg-slate-950/60 border border-slate-800">
                  <span className="text-xs font-semibold uppercase text-slate-400 tracking-wider flex items-center mb-2">
                    <UserCheck className="w-3.5 h-3.5 mr-1 text-emerald-400" /> Chitresh's Resume
                  </span>
                  <p className="text-sm text-slate-200">{selectedJob.candidate_experience || "Analyzed from resume"}</p>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-amber-950/10 border border-amber-500/20">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-amber-400 flex items-center mb-2">
                  <AlertTriangle className="w-4 h-4 mr-1.5" /> Experience Gap
                </h4>
                <p className="text-sm text-slate-300 leading-relaxed">{selectedJob.experience_gap || "No major gap detected."}</p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-rose-950/10 border border-rose-500/20">
                ...
                <p className="text-sm text-slate-300 leading-relaxed">{selectedJob.missing_skills || "Skills match closely."}</p>
              </div>
              ...
              <div className="p-4 rounded-xl bg-indigo-950/20 border border-indigo-500/20">
                ...
                <p className="text-sm text-slate-300 leading-relaxed">{selectedJob.overall_gap_summary || "Good match."}</p>
              </div>
            </div>
          ) : (
